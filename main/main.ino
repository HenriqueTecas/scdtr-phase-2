...
enum class CalStage : uint8_t {
    CAL_BARRIER, CAL_BACKGROUND,
    CAL_ACTIVE_ON, CAL_ACTIVE_SAMPLE, CAL_ACTIVE_WAIT,
    CAL_PASSIVE_WAIT, CAL_PASSIVE_SETTLE, CAL_DONE
};
...
// =============================================================================
// cal_build_node_list — luminaire indices, sorted ascending
// =============================================================================
void cal_build_node_list() {
    ...
    for (int i = 1; i < N_NODES; i++) {
        uint8_t key = all_nodes[i]; int j = i - 1;
        while (j >= 0 && all_nodes[j] > key) { all_nodes[j+1]=all_nodes[j]; j--; }
        all_nodes[j + 1] = key;
    }
}
...
void network_wakeup() {
    ...
    while (n_other_nodes < N_NODES - 1) {
        ...
        if (!queue_try_remove(&can_rx_queue, &frm)) continue;
        if (frm.can_id == 0xFFFFFFFF) continue;

        uint8_t type = CAN_ID_TYPE(frm.can_id);
        uint8_t src  = CAN_ID_SRC(frm.can_id);
        uint8_t sub  = frm.data[0];

        if (type != MSG_WAKEUP) { queue_try_add(&can_rx_queue, &frm); continue; }
...
void loop1() {
    ...
    if (!can_got_irq && gpio_get(CAN_INT_PIN)) return;
    can_got_irq = false;
    ...
    while (queue_try_remove(&can_rx_queue, &frm)) {
        ...
        case MSG_SET: {
            ...
            case 'r': {
                ...
                r = val; flicker_holdoff = FLICKER_EXCLUDE_SAMPLES;
            ...
            case 's': {
                ...
                if      (var == 'y') stream_y = on_off;
                else if (var == 'u') stream_u = on_off;
                else if (var == 'j') stream_j = on_off;
                can_send_byte(src, MSG_CTRL, SUB_ACK, (uint8_t)LUMINAIRE);
                break;
            }
            }
        ...
        case MSG_GET: {
            ...
            float val = 0;
            switch ((char)sub) {
            case 'y': val = snap_lux; break;
            case 'u': val = pid.get_feedback() ? pid.get_duty_cycle() : serial_duty_cycle; break;
            case 'r': val = r; break;
            case 'p': val = pid.get_u() * MAXIMUM_POWER; break;
            case 'd': val = max(0.0f, snap_lux - sys_gain * snap_duty); break;
            case 't': val = micros() * 1e-6f; break;
            case 'E': val = (float)energy_consumption; break;
            case 'V': val = sample_count ? (float)(visibility_error/(double)sample_count) : 0.0f; break;
            case 'F': val = sample_count ? (float)((flicker_error/(double)sample_count)/pid.h) : 0.0f; break;
            case 'O': val = ref_high; break;
            case 'U': val = ref_low; break;
            case 'L': val = (pid.get_occupancy()==2)?ref_high:(pid.get_occupancy()==1)?ref_low:0.0f; break;
            case 'C': val = energy_cost; break;
            case 'o': val = (float)pid.get_occupancy(); break;
            case 'a': val = (float)pid.get_anti_windup(); break;
            case 'f': val = (float)pid.get_feedback(); break;
            case 'v': val = (analogRead(LDR_PIN) / (float)ADC_MAX) * VCC; break;
            }
        ...
        case MSG_CAL: {
            ...
            case SUB_CAL_LUX:
                if (cal_stage == CalStage::CAL_ACTIVE_WAIT  ||
                    cal_stage == CalStage::CAL_ACTIVE_ON    ||
                    cal_stage == CalStage::CAL_ACTIVE_SAMPLE) {
                    float peer_lux; memcpy(&peer_lux, frm.data + 1, 4);
...
void hub_forward(const char* cmd_str, uint8_t dest_node) {
    ...
    if (command == 'R') {
        ...
        delay(500); rp2040.reboot(); return;
    }
...
void setup() {
    ...
    for (int i = 0; i < N_NODES; i++) {
        if (UID_TABLE[i] == uid) { LUMINAIRE = i + 1; break; }
    }
    if (LUMINAIRE == 0) {
        while (true) {
