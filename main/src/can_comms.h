...
#include "mcp2515.h"        // local MCP2515 library
...
// ── Node address (assigned dynamically at boot via flash unique ID) ────────
...
inline MCP2515::ERROR can_broadcast_lux(float lux, float duty, float ref)
{
    extern volatile uint32_t can_tx_count;
    can_tx_count++;  // Increment TX counter
...
// are seen or after CAN_DISCOVER_IDLE seconds of no new peer.
// Smallest signature → node 1, next → node 2, largest → node 3.
#define CAN_DISCOVER_NODES   3
#define CAN_DISCOVER_TIMEOUT 60
#define CAN_DISCOVER_IDLE    15   // wait 15 s with no new peer before giving up
...
static void can_assign_id()
{
    ...
    for (int sec = 0; sec < CAN_DISCOVER_TIMEOUT; sec++) {
        ...
        if (n_sigs == CAN_DISCOVER_NODES) break;
    ...
    // Sort ascending so smallest signature → node 1.
...
{
    // Phase 1: initial MCP2515 bring-up (needed for can_assign_id to transmit)
    ...
    // Phase 2: dynamic node assignment via flash ID negotiation
    can_assign_id();

    // Safety fallback: if no peers found at all, default to node 1
    if (node_address == 0) {
        node_address = 1;
        LUMINAIRE    = 1;
    ...
    // Phase 3: re-init MCP2515 to flush TX buffers jammed by unACKed discovery frames.
    // Every ANNOUNCE frame during single-node discovery is never ACKed, so TX error
    // counter builds up and TX buffers end up stuck (ERROR_ALLTXBUSY).
