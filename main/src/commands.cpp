...
extern float serial_duty_cycle;
extern PID pid;
extern const int LED_PIN;
extern const int PWM_MAX;
extern float r;
extern int flicker_holdoff;
extern const int FLICKER_EXCLUDE_SAMPLES;
extern int stream_y, stream_u, stream_j;
extern float lux_value;
extern float duty_cycle;
extern int buffer_y, buffer_u, buffer_read_size, buffer_read_counter;
extern CircularBuffer<6000> last_min_buf;
...
// =============================================================================
{
    ...
    case 'u':
        sscanf(buffer, "%c %d %f", &command, &luminaire_index, &value);
    ...
    case 's':
        sscanf(buffer, "%c %c %d", &command, &x_variable, &luminaire_index);
        if (LUMINAIRE == luminaire_index) {
            if      (x_variable == 'y') stream_y = 1;
            else if (x_variable == 'u') stream_u = 1;
            else if (x_variable == 'j') stream_j = 1;
        }
        break;
    ...
    case 'c':
        sscanf(buffer, "%c %c", &command, &sub_command);
        switch (sub_command)
        {
        case 'b': {
            float new_b;
            if (sscanf(buffer + 3, "%f", &new_b) == 1) {
                ldr_b = new_b;
        ...
        case 's': {
            int steps = 10;
            if (strlen(buffer) > 3) sscanf(buffer + 3, "%d", &steps);
        ...
        case '?':
            ...
            break;
        case 'p': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
            if (LUMINAIRE==idx){ pid.set_kp(val); PRINTF(out,"ack: kp=%.4f\n",val); }
            else out.println("err"); break; }
        case 'i': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
            if (LUMINAIRE==idx){ pid.set_ki(val); PRINTF(out,"ack: ki=%.4f\n",val); }
            else out.println("err"); break; }
        case 'B': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
            if (LUMINAIRE==idx){ pid.set_b(val);  PRINTF(out,"ack: b=%.4f\n", val); }
            else out.println("err"); break; }
        case 't': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
            if (LUMINAIRE==idx){ pid.set_kt(val); PRINTF(out,"ack: kt=%.4f\n",val); }
            else out.println("err"); break; }
        case 'w': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
            if (LUMINAIRE==idx){ pid.set_bumpless((int)val); PRINTF(out,"ack: bumpless=%d\n",(int)val); }
            else out.println("err"); break; }
        case 'f': { float val; int idx; sscanf(buffer+3,"%d %f",&idx,&val);
    ...
    // ── g : Get variable ───────────────────────────────────────────────────────
    case 'g':
        sscanf(buffer, "%c %c %d", &command, &sub_command, &luminaire_index);
        switch (sub_command)
        {
    ...
    case 'h':
        ...
        out.println("CAN:      G  (show CAN stats)");
    ...
    case 'G':
        {
            ...
            PRINTF(out, "=== CAN Status (node %d) ===\n", LUMINAIRE);
            PRINTF(out, "  TX total: %lu\n", can_tx_count);
            PRINTF(out, "  RX total: %lu\n", can_rx_count);
            PRINTF(out, "  RX hello: %lu\n", can_rx_hello);
            PRINTF(out, "  RX lux:   %lu\n", can_rx_lux);
            PRINTF(out, "  RX cmd:   %lu\n", can_rx_cmd);
        }
        break;
