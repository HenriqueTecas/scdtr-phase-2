...
void hub_forward_command(const char *cmd, uint8_t dest_node)
{
    if (can_send_cmd(dest_node, cmd) != MCP2515::ERROR_OK)
        Serial.printf("[CAN] forward to node %d failed\n", (int)dest_node);
    // No immediate echo – we wait for the remote node's response.
}

// ── Dispatch one received CAN message ────────────────────────────────────
// Call this in the main loop for every message returned by can_poll().
void hub_dispatch_msg(const CanRxMsg &msg)
{
    if (!msg.valid) return;
